...
__export(transations_exports, {
  transationsRoutes: () => transationsRoutes
});
...
// src/routes/transations.ts
async function transationsRoutes(app) {
  ...
  app.get(
    ...
    async (request) => {
      const { sessionId } = request.cookies;
      const summary = await knex("transactions").andWhere("session_id", sessionId).sum("amount", { as: "amount" }).first();
      return { summary };
    }
  );
  ...
  app.delete("/:id", async (request) => {
    ...
    const { sessionId } = request.cookies;
    const { id } = getTransactionParamsSchema.parse(request.params);
    await knex("transactions").where("id", id).andWhere("session_id", sessionId).delete();
  });
}
